  ...
  return (
    <div className={`flex h-screen w-screen overflow-hidden select-none ${bg}`}>
      ...
      <main className="flex-1 min-w-0 p-2 sm:p-3 relative" onClick={() => { if (leftOpen || rightOpen) closeAll(); }}>
        ...
        {/* Floating action bar */}
        <div
          className="absolute top-5 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-3 py-2 rounded-full shadow-md border pointer-events-auto"
          style={{
        ...
        >
          ...
          <button
            ...
            title="Surahs"
            className="sm:hidden p-2.5 rounded-full transition-all hover:opacity-70"
            style={{ color: accent }}
          >
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 sm:px-4 py-2.5 rounded-full text-sm font-semibold transition-all"
...
function Sep({ isDark }: { isDark: boolean }) {
  return <div style={{ width: 1, height: 28, background: isDark ? "#2a2a4e" : "#d8d3c0", flexShrink: 0 }} />;
}
...
function ActionBtn({
  ...
  return (
    <button
      ...
      title={title}
      disabled={disabled}
      className="p-2.5 rounded-full transition-all hover:opacity-70 disabled:opacity-25"
      style={{ color }}
    >
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        {children}
