  ...
  return (
    ...
    >
      ...
      <div className="border-t pt-3" style={{ borderColor: isDark ? "#2a2a4e" : "#e8e3d5" }}>
        <p className={`text-xs font-semibold uppercase tracking-wider mb-2 ${mutedText}`}>
          Browse Surahs
        </p>
        ...
        <input
          ...
          className={`w-full px-3 py-2 rounded-xl text-xs outline-none border mb-2 ${
            isDark
        ...
        {loading && safeChapters.length === 0 ? (
          <div className={`flex items-center justify-center py-6 gap-2 ${mutedText} text-xs`}>
          ...
          <div className="flex flex-col gap-0.5 overflow-y-auto pr-0.5" style={{ maxHeight: "calc(100vh - 320px)" }}>
            {filtered.map((ch) => (
              <button
                key={ch.id}
